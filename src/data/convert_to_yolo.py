...
#     Returns:
#         boxes   : list of valid (class_id_raw, x, y, w, h)
#         skipped : count of lines skipped due to score=0, invalid class, or bad box
#     """
#     boxes   = []
#     skipped = 0
#     with open(ann_path, "r") as f:
...
#                 continue
#             try:
#                 x, y, w, h, score, cat = int(parts[0]), int(parts[1]), \
...
#     Convert VisDrone boxes to YOLO normalized format.
#     Input  : (cat, x_topleft, y_topleft, w, h) in pixels
#     Output : (yolo_class_id, cx, cy, w_norm, h_norm) normalized [0, 1]
#     """
#     yolo_lines = []
#     for cat, x, y, w, h in boxes:
#         cx = (x + w / 2) / img_w
...
#         yolo_class = LABEL_MAP[cat]
#         yolo_lines.append(f"{yolo_class} {cx:.6f} {cy:.6f} {w_norm:.6f} {h_norm:.6f}")
#     return yolo_lines
...
#         if len(raw_boxes) == 0:
#             # write empty label file — YOLO expects file to exist
#             label_path = label_dir / (ann_file.stem + ".txt")
#             label_path.write_text("")
#             total_images  += 1
#             skipped_boxes += n_skipped
